...
class BuilderBotWhatsAppNotificationService implements WhatsAppNotificationService
{
    ...
    public function sendComplaintMessage(Complaint $complaint, string $type, array $context = []): array
    {
        ...
        if ($type === ComplaintStatus::Resolved->value && filled($context['resolution_photo_url'] ?? null)) {
            $payload['messages']['mediaUrl'] = (string) $context['resolution_photo_url'];
        }
    ...
    private function bodyMessage(Complaint $complaint, string $type, array $context): string
    {
        ...
        $trackingUrl = URL::route('complaints.public.received', $complaint);
        ...
        return match ($type) {
            ...
            ComplaintStatus::UnderReview->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} paso a estado \"{$statusLabel}\". El equipo municipal esta verificando la informacion cargada.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::Assigned->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} paso a estado \"{$statusLabel}\". Ya fue derivado a una cuadrilla municipal para su atencion.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::InProgress->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} paso a estado \"{$statusLabel}\". La cuadrilla se encuentra trabajando sobre la solicitud.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::NeedsSecondVisit->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} requiere una segunda visita de la cuadrilla para completar la solucion.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::Resolved->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} fue resuelto por la cuadrilla municipal. Muchas gracias por colaborar con el cuidado del alumbrado publico.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::Closed->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} fue cerrado administrativamente.{$observation}\n\nSeguimiento: {$trackingUrl}",
            ComplaintStatus::Cancelled->value => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} fue cancelado.{$observation}\n\nSeguimiento: {$trackingUrl}",
            default => "Estimado/a vecino/a {$neighborName}, le informamos que su reclamo {$complaint->public_code} paso a estado \"{$statusLabel}\".{$observation}\n\nSeguimiento: {$trackingUrl}",
        };
    }
